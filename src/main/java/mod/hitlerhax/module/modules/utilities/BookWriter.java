...
public class BookWriter extends Module {
	...
	public StringSetting path = new StringSetting("TXT Path", this, "C:\\example.txt");
	IntSetting delay = new IntSetting("Typing Delay (ms)", this, 100);
	...
	public void actionPerformed(GuiButton button, NBTTagList bookPages, int currPage, boolean bookIsModified) {
		if (button.id == 69420) {
			// clear strings
			fileContents = "";
			written = "";
			...
			for (int i = 0; i < fileContents.length(); i++) {
				...
				int height = mc.fontRenderer.getWordWrappedHeight(currentPage + i + "" + TextFormatting.BLACK + "_",
						118);
				if (height <= 128 && currentPage.length() < 256) {
	...
	public void drawScreen(boolean bookIsUnsigned, int width, int height, int mouseX, int mouseY, float partialTicks) {
		if (this.isToggled() && bookIsUnsigned) {
			...
			mc.fontRenderer.drawSplitString(
					written.length() < 100 ? written : written.substring(written.length() - 100, written.length()), 10,
			...
			mc.fontRenderer.drawSplitString(fileContents.length() < 250 ? fileContents : fileContents.substring(0, 250),
					...
					mc.fontRenderer.getWordWrappedHeight(written.length() < 100 ? written
							: written.substring(written.length() - 100, written.length()), width / 8) + 20,
