...
public class SettingButton {
	...
	public void draw(int mouseX, int mouseY) {
		...
		if (setting instanceof ModeSetting) {
			...
			FontUtils.drawString(cSetting.name + ": ", x + 2, y + 2,
					new ColorUtil(cSetting.red, cSetting.green, cSetting.blue));
			int textRed = (int) FontUtils.drawString(Integer.toString(cSetting.red),
					x + 2 + FontUtils.getStringWidth(cSetting.name + ": "), y + 2, new ColorUtil(255, 0, 0));
			int textGreen = (int) FontUtils.drawString(Integer.toString(cSetting.green),
					x + 2 + FontUtils.getStringWidth(cSetting.name + ": ") + 125, y + 2, new ColorUtil(0, 255, 0));
			int textBlue = (int) FontUtils.drawString(Integer.toString(cSetting.blue),
	...
	public void onClick(int x, int y, int button) {
		...
		if (x >= this.x && x <= this.x + this.width && y >= this.y && y <= this.y + this.height) {
			...
			HtlrEventBus.EVENT_BUS.post(new HtlrEventSettings(setting, setting.parent));
